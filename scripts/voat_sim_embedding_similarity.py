...
Usage (pyenv):
  PYENV_VERSION=ysocial python scripts/voat_sim_embedding_similarity.py \
    ...
    [--mode both|posts|comments] [--no-plot-tsne] [--no-plot-umap]
...
def maybe_apply_instruction_prefix(
    model_name: str, texts: List[str], is_query: bool
) -> List[str]:
...
# Data loading
# -------------------------
def load_sim2_subset(
    posts_csv: Path, tox_csv: Path, want_comments: bool
) -> pd.DataFrame:
...
def load_voat_subset(parquet_path: Path, want_comments: bool) -> pd.DataFrame:
    ...
    df = pd.read_parquet(
        parquet_path, columns=["post_id", "content", "interaction_type"]
    )
...
def nearest_by_cosine(
    corpus_embs: np.ndarray, query_embs: np.ndarray
...
def compute_stats(
    best_scores: np.ndarray, thr_mid: float, thr_high: float
) -> SimilarityStats:
...
def maybe_plot_2d(
    ...
    try:
        ...
        if corpus_embs.shape[0] > max_corpus_points:
            ...
            idx = rng.choice(
                corpus_embs.shape[0], size=max_corpus_points, replace=False
            )
        ...
        for cls, color, lab in [
            (0, colors[0], labels_full[0]),
            (1, colors[1], labels_full[1]),
        ]:
            pts = Z[y == cls]
            plt.scatter(
                pts[:, 0],
                pts[:, 1],
                s=8 if cls == 1 else 20,
                alpha=0.7 if cls == 1 else 0.9,
                label=lab,
                c=color,
            )
        plt.legend()
...
def run_one(
    ...
    if remove_top_k_tokens > 0:
        ...
        def strip_common(doc: str) -> str:
            return " ".join(
                [w for w in token_re.findall(doc.lower()) if w not in common]
            )
...
def main(argv: Optional[List[str]] = None) -> int:
    p = argparse.ArgumentParser(
        description="Embedding similarity between simulation and Voat (posts and/or comments)"
    )
    ...
    p.add_argument(
        "--mode", type=str, choices=["both", "posts", "comments"], default="both"
    )
    p.add_argument(
        "--device", type=str, default=None, help="Force device for embeddings: cuda/cpu"
    )
    p.add_argument("--embedding-model", type=str, default="intfloat/e5-large-v2")
    ...
    p.add_argument(
        ...
        dest="plot_tsne",
        action="store_true",
        help="Enable t-SNE scatter plots (default: enabled)",
    )
    p.add_argument(
        "--no-plot-tsne",
        dest="plot_tsne",
        action="store_false",
        help="Disable t-SNE scatter plots.",
    )
    p.set_defaults(plot_tsne=True)
    p.add_argument(
        "--tsne-perplexities",
        type=str,
        default="3,30,80",
        help="Comma-separated perplexities for t-SNE when plotting",
    )
    p.add_argument(
        "--remove-top-k-tokens",
        type=int,
        default=0,
        help="Remove top K most frequent tokens (document frequency) across corpora before embedding (0 disables)",
    )
    p.add_argument(
        "--plot-umap",
        dest="plot_umap",
        action="store_true",
        help="Enable UMAP scatter plots (default: enabled)",
    )
    p.add_argument(
        "--no-plot-umap",
        dest="plot_umap",
        action="store_false",
        help="Disable UMAP scatter plots.",
    )
    p.set_defaults(plot_umap=True)
    p.add_argument(
        "--plot-sample-size",
        type=int,
        default=2000,
        help="Max corpus points to include in plots",
    )
    ...
    tsne_perplexities = [
        int(x) for x in str(args.tsne_perplexities).split(",") if x.strip()
    ]
    ...
    if args.mode in {"both", "comments"}:
        sim_comments = load_sim2_subset(
            args.sim2_posts, args.sim2_tox, want_comments=True
        )
    ...
    if args.mode in {"both", "posts"}:
        sim_posts = load_sim2_subset(
            args.sim2_posts, args.sim2_tox, want_comments=False
        )
