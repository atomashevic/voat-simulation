...
of a simulation run with Voat technology samples, focusing on similarities
alongside obvious differences.

Usage:
    python scripts/comparative_network_analysis_voat.py
    python scripts/comparative_network_analysis_voat.py --sim-dir simulation3

Output:
    - All outputs are saved under <sim-dir>/voat-comparison/
      - Comparative visualizations: <sim-dir>/voat-comparison/plots/
      - CSV summaries: <sim-dir>/voat-comparison/
      - Text report: <sim-dir>/voat-comparison/similarity_report.txt
"""
...
class VoatNetworkComparator:
    ...
    def __init__(self, base_path=".", sim_dir="simulation", voat_dir="MADOC/voat-technology"):
        """
        Initialize the comparator with base paths.
        
        Args:
            base_path: Base directory path (default: current directory)
            sim_dir: Directory containing simulation outputs (default: "simulation")
            voat_dir: Directory containing Voat samples (default: "MADOC/voat-technology")
        """
        self.base_path = Path(base_path).resolve()
        sim_dir_path = Path(sim_dir)
        voat_dir_path = Path(voat_dir)

        self.simulation_path = sim_dir_path if sim_dir_path.is_absolute() else (self.base_path / sim_dir_path)
        self.voat_path = voat_dir_path if voat_dir_path.is_absolute() else (self.base_path / voat_dir_path)
        # Save all outputs under the chosen simulation directory for easier packaging
        self.output_path = self.simulation_path / "voat-comparison"
        ...
        sim_dir_name = self.simulation_path.name or "simulation"
        self.sim_label = ("Voat Simulation" if sim_dir_name == "simulation"
                           else f"Voat Simulation ({sim_dir_name})")
    ...
    def load_simulation_data(self):
        ...
        print(f"Loading simulation data from {self.simulation_path}...")
        
        # Load core-periphery membership (support flat-run naming)
        cp_candidates = [
            self.simulation_path / 'enhanced_core_periphery_membership.csv',
            self.simulation_path / 'core_periphery_membership.csv',
            self.simulation_path / 'core_periphery.csv',
        ]
        for cp_path in cp_candidates:
            if cp_path.exists():
                self.simulation_data['core_periphery'] = pd.read_csv(cp_path)
                break
        
        # Load centrality measures
        centrality_file = self.simulation_path / "centrality_measures.csv"
        if centrality_file.exists():
            self.simulation_data['centrality'] = pd.read_csv(centrality_file)
            
        # Parse network statistics (fall back to enhanced analysis log if needed)
        stats_candidates = [
            self.simulation_path / "network_stats_cp.txt",
            self.simulation_path / "enhanced_network_analysis.txt",
            self.simulation_path / "network_analysis.txt",
        ]
        for stats_file in stats_candidates:
            if stats_file.exists():
                self.simulation_data['network_stats'] = self._parse_network_stats(stats_file)
                break
            
        print(f"  - Loaded simulation data: {len(self.simulation_data)} datasets")
    ...
    def load_voat_samples(self):
        ...
        for sample_dir in sample_dirs:
            ...
            stats_candidates = [
                sample_dir / "network_stats_cp.txt",
                sample_dir / "enhanced_network_analysis.txt",
            ]
            for stats_file in stats_candidates:
                if stats_file.exists():
    ...
    def calculate_network_similarities(self):
        ...
        if not self.simulation_data or not self.voat_samples:
            raise ValueError("Data not loaded. Call load_simulation_data() and load_voat_samples() first.")
        
        sim1_stats = self.simulation_data.get('network_stats', {})
        similarities = {}
        ...
        for sample_name, sample_data in self.voat_samples.items():
            ...
            if ('core_periphery' in self.simulation_data and 
                'core_periphery' in sample_data):
                
                sim1_cp = self.simulation_data['core_periphery']
    ...
    def _plot_degree_distributions_separate(self, plot_dir):
        ...
        sim1_cp = self.simulation_data.get('core_periphery')
        if sim1_cp is None:
            return
    ...
    def _plot_post_distribution_kde(self, plot_dir: Path, sample_name: str = "sample_1") -> None:
        ...
        try:
            ...
            sim_posts_csv = self.simulation_path / "posts.csv"
    ...
    def _plot_toxicity_kde(self, plot_dir: Path, sample_name: str = "sample_1") -> None:
        ...
        try:
            # Simulation toxicity
            sim_tox_csv = self.simulation_path / "toxigen.csv"
    ...
    def _export_user_comparisons(self):
        ...
        # Simulation users
        if 'core_periphery' in self.simulation_data:
            sim1_cp = self.simulation_data['core_periphery']
    ...
    def _plot_core_periphery_networks(self, plot_dir):
        ...
        self._create_network_visualization('Simulation', self.simulation_path / "posts.csv", 
                                         self.simulation_data, plot_dir)
    ...
    def generate_similarity_report(self):
        ...
        with open(report_path, 'w') as f:
            ...
            f.write(f"{self.sim_label} vs Voat Technology Samples\n")
            f.write("=" * 80 + "\n\n")
            ...
            sim1_stats = self.simulation_data.get('network_stats', {})
            f.write(f"{self.sim_label} Network:\n")
...
def main():
    """Parse CLI arguments and run the comparative analysis."""
    parser = argparse.ArgumentParser(
        description="Compare simulation outputs against Voat samples."
    )
    parser.add_argument(
        "--sim-dir",
        default="simulation",
        help="Directory containing simulation outputs (default: simulation)",
    )
    parser.add_argument(
        "--voat-dir",
        default="MADOC/voat-technology",
        help="Directory containing Voat samples (default: MADOC/voat-technology)",
    )
    parser.add_argument(
        "--base-path",
        default=".",
        help="Base path used to resolve relative directories (default: current directory)",
    )
    args = parser.parse_args()

    comparator = VoatNetworkComparator(
        base_path=args.base_path, sim_dir=args.sim_dir, voat_dir=args.voat_dir
    )
