...
def plot_lag_entropy_scatter(
    ...
    """Scatter all values (alpha) for lag vs entropy with optional filtering and mean trend lines."""
    if df.empty or value_col not in df.columns:
        return

    subset = df if pair_type is None else df[df["pair_type"] == pair_type]
    if subset.empty:
        return

    pair_types: Sequence[str]
    if pair_type is None:
        pair_types = [ptype for ptype in ("interpersonal", "intrapersonal") if ptype in subset["pair_type"].unique()]
    else:
    ...
    stored_series: List[pd.Series] = []
    shown_labels: set[str] = set()

    for ptype in pair_types:
        grp = subset[subset["pair_type"] == ptype].copy()
        if grp.empty:
            continue
        grp["count_per_lag"] = grp.groupby("lag")[value_col].transform("size")
        scatter_data = grp[grp["count_per_lag"] >= min_count]
        if scatter_data.empty:
            continue
        stored_series.append(scatter_data[value_col])
        color = palette.get(ptype, "#1f77b4")
        label = ptype.capitalize()
        label_to_use = None
        if pair_type is None and label not in shown_labels:
            label_to_use = label
            shown_labels.add(label)
        ax.scatter(
            scatter_data["lag"],
            scatter_data[value_col],
            color=color,
            alpha=0.25,
            s=16,
            edgecolor="none",
        ...
        mean_series = (
            scatter_data.groupby("lag")[value_col]
            .mean()
            .reset_index()
            .sort_values("lag")
        )
        ax.plot(mean_series["lag"], mean_series[value_col], color=color, linewidth=2.2)

    if not stored_series:
        plt.close(fig)
        return

    combined = pd.concat(stored_series, ignore_index=True)
    ...
    ax.set_title(title)
    if pair_type is None and shown_labels:
...
def run(posts_csv: Path, outdir: Path, model_name: str, device: str, sigma: float, max_tokens: int, max_lag: int, min_chain_len: int) -> int:
    ...
    try:
        ...
        plot_lag_entropy_scatter(
            df,
            value_col="H",
            ylabel="Convergence entropy H",
            title="Lag vs Convergence Entropy (H)",
            outpath=outdir / "lag_vs_entropy.png",
            palette=pal,
        )
        plot_lag_entropy_scatter(
            df,
            value_col="H_per_token",
            ylabel="Convergence entropy per token (H/T_x)",
            title="Lag vs Convergence Entropy (per token)",
            outpath=outdir / "lag_vs_entropy_per_token.png",
            palette=pal,
        )
        for ptype in ("interpersonal", "intrapersonal"):
            title_suffix = ptype.capitalize()
            plot_lag_entropy_scatter(
                df,
                value_col="H",
                ylabel="Convergence entropy H",
                title=f"Lag vs Convergence Entropy (H) – {title_suffix}",
                outpath=outdir / f"lag_vs_entropy_{ptype}.png",
                palette=pal,
                pair_type=ptype,
            )
            plot_lag_entropy_scatter(
                ...
                value_col="H_per_token",
                ylabel="Convergence entropy per token (H/T_x)",
                title=f"Lag vs Convergence Entropy (per token) – {title_suffix}",
                outpath=outdir / f"lag_vs_entropy_per_token_{ptype}.png",
                palette=pal,
                pair_type=ptype,
            )
