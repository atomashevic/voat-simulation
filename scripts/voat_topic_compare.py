...
def clean_text(s: str) -> str:
    ...
    # Remove repeated column prefixes / metadata markers anywhere in the string
    s = re.sub(r"\b(title|body|content|text|selftext|tweet)\s*:\s*", " ", s, flags=re.IGNORECASE)
    ...
    s = re.sub(r"^[.@]\S+\s+", "", s)
    # Remove residual HTML-like brackets
    s = re.sub(r"<[^>]+>", " ", s)
    # Collapse emphasis markers and stray punctuation
    s = re.sub(r"[\"“”‘’]+", " ", s)
    s = WS_RE.sub(" ", s)
    s = s.strip(" '\"")
    if s.isupper() and len(s) > 12:
        s = s.lower()
    # Drop extremely short boilerplate words
    tokens = s.split()
    if tokens and tokens[0].lower() in {"title", "body", "content"}:
        tokens = tokens[1:]
    s = " ".join(tokens)
    return s.strip()
...
TOKENIZE_RE = re.compile(r"[A-Za-z][A-Za-z0-9_'\\-]+")


def _tokenize_unique(doc: str) -> set:
    if not doc:
        return set()
    return set(TOKENIZE_RE.findall(doc.lower()))


def compute_dynamic_stopwords(
    texts: List[str],
    min_token_freq: float,
    df_threshold: float,
    *,
    min_docs_required: int,
) -> set:
    """Derive additional stopwords based on per-corpus document frequency."""
    doc_count = len(texts)
    if doc_count < max(1, min_docs_required):
        return set()
    token_counts: Counter[str] = Counter()
    for doc in texts:
        token_counts.update(_tokenize_unique(doc))
    extra = set()
    if min_token_freq and min_token_freq > 0:
        if min_token_freq < 1:
            cutoff = max(2, math.ceil(min_token_freq * doc_count))
        else:
            cutoff = max(2, int(min_token_freq))
        for tok, cnt in token_counts.items():
            if cnt >= cutoff:
                extra.add(tok)
    if df_threshold and 0 < df_threshold < 1:
        cutoff = max(2, math.ceil(df_threshold * doc_count))
        for tok, cnt in token_counts.items():
            if cnt >= cutoff:
                extra.add(tok)
    return extra
...
def adaptive_min_topic_size(doc_count: int, requested: int) -> int:
    if doc_count <= 0:
        return max(2, requested)
    if requested <= 2:
        return max(2, requested)
    if doc_count < requested * 2:
        return max(2, min(requested, max(2, doc_count // 5 or 1)))
    return requested


def adaptive_min_df(doc_count: int, requested: int) -> int:
    if requested <= 1 or doc_count <= requested:
        return 1
    return requested
...
def build_stopword_set(
    ...
    *,
    ...
    stopwords = set(base_extra)
    stopwords.update(c.lower() for c in column_names)
    stopwords.update(
        compute_dynamic_stopwords(
            texts,
...
def apply_stopwords(topic_model, extra_stop: set) -> None:
    if not extra_stop or not hasattr(topic_model, "vectorizer_model"):
        return
    base_stop = set(topic_model.vectorizer_model.get_stop_words() or [])
    topic_model.vectorizer_model.stop_words = list(base_stop.union(extra_stop))
...
def build_bertopic(
    ...
    if hdbscan_kwargs:
        hdbscan_params.update(hdbscan_kwargs)
...
class SimpleTopicModel:
    """Lightweight topic model fallback for tiny corpora."""

    def __init__(self, seed: int, min_topic_size: int, top_n_words: int):
        self.seed = seed
        self.min_topic_size = max(2, min_topic_size)
        self.top_n_words = max(5, top_n_words)
    ...
    def fit_transform(self, texts: List[str]):
        if not texts:
            self._assignments = np.array([], dtype=int)
            self._topic_info_df = pd.DataFrame(columns=["Topic", "Count", "Name"])
            self._labels = {}
            return self._assignments, None
        vectorizer = CountVectorizer(stop_words="english", min_df=1)
        try:
            X = vectorizer.fit_transform(texts)
        except ValueError:
            # Last resort: treat entire corpus as one topic without vectorizer support
            self._assignments = np.zeros(len(texts), dtype=int)
            self._topics = {
                0: {
                    "words": [],
                    "count": len(texts),
                    "label": "Manual Topic 0",
                }
            }
            self._topic_info_df = pd.DataFrame([{"Topic": 0, "Count": len(texts), "Name": "Manual Topic 0"}])
            self._labels = {0: "Manual Topic 0"}
            return self._assignments, None
        doc_count = len(texts)
        n_topics = max(1, min(doc_count // self.min_topic_size, min(8, doc_count)))
        if n_topics <= 1:
            labels = np.zeros(doc_count, dtype=int)
        else:
            kmeans = KMeans(n_clusters=n_topics, random_state=self.seed, n_init="auto")
        ...
        for tid in sorted(set(labels)):
            ...
            if idxs.size == 0:
                continue
            topic_counts = np.asarray(X[idxs].sum(axis=0)).ravel()
            order = topic_counts.argsort()[::-1]
            words_scores = [
                (feature_names[i], float(topic_counts[i])) for i in order if topic_counts[i] > 0
            ][: max(self.top_n_words * 2, 20)]
            if not words_scores:
                continue
            topics[tid] = {
                "words": words_scores,
                "count": int(idxs.size),
                "label": f"Manual Topic {tid}",
            }
        if not topics:
            sums = np.asarray(X.sum(axis=0)).ravel()
            order = sums.argsort()[::-1]
            words_scores = [
                (feature_names[i], float(sums[i])) for i in order if sums[i] > 0
            ][: max(self.top_n_words * 2, 20)]
            topics[0] = {"words": words_scores, "count": doc_count, "label": "Manual Topic 0"}
            labels = np.zeros(doc_count, dtype=int)
        self._topics = topics
        self._assignments = labels
        info_rows = [
            {"Topic": tid, "Count": data["count"], "Name": data["label"]} for tid, data in topics.items()
        ]
        self._topic_info_df = pd.DataFrame(info_rows)
        self._labels = {tid: data["label"] for tid, data in topics.items()}
        return labels, None

    def transform(self, texts: List[str]):
        if self._assignments is None:
            raise ValueError("SimpleTopicModel must be fit before calling transform.")
        return self._assignments, None

    def get_topic_info(self):
        return self._topic_info_df if self._topic_info_df is not None else pd.DataFrame(columns=["Topic", "Count", "Name"])

    def get_topic_labels(self):
        return self._labels

    def get_topic(self, topic_id: int):
        return self._topics.get(topic_id, {}).get("words", [])
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    parser.add_argument("--sim2-vectorizer-min-df", type=int, default=None, help="Override vectorizer min_df for the simulation corpus")
    parser.add_argument("--madoc-vectorizer-min-df", type=int, default=None, help="Override vectorizer min_df for the MADOC corpus")
    ...
    parser.add_argument("--sim2-df-threshold", type=float, default=None, help="Override df-threshold for simulation corpus dynamic stopwords")
    parser.add_argument("--madoc-df-threshold", type=float, default=None, help="Override df-threshold for MADOC corpus dynamic stopwords")
    ...
    parser.add_argument("--sim2-min-token-freq", type=float, default=None, help="Override min-token-freq for simulation corpus stopwords")
    parser.add_argument("--madoc-min-token-freq", type=float, default=None, help="Override min-token-freq for MADOC corpus stopwords")
    parser.add_argument("--sim2-min-topic-size", type=int, default=None, help="Min topic size for the simulation BERTopic model")
    parser.add_argument("--madoc-min-topic-size", type=int, default=None, help="Min topic size for the MADOC BERTopic model")
    parser.add_argument("--sim2-min-docs", type=int, default=None, help="Warning threshold for too-small simulation corpora")
    parser.add_argument("--madoc-min-docs", type=int, default=None, help="Warning threshold for too-small MADOC corpora")
    parser.add_argument("--sim2-max-docs", type=int, default=None, help="Optional cap on simulation documents (overrides --max-docs)")
    parser.add_argument("--madoc-max-docs", type=int, default=20000, help="Optional cap on MADOC documents (overrides --max-docs)")
    ...
    sim2_max_docs = args.sim2_max_docs if args.sim2_max_docs is not None else args.max_docs
    madoc_max_docs = args.madoc_max_docs if args.madoc_max_docs is not None else args.max_docs

    # Load corpora
    sim2_texts, sim2_df, sim2_used_cols = load_corpus(args.sim2_posts_csv, text_cols=sim2_text_cols, max_docs=sim2_max_docs, seed=args.seed)
    sim2_min_docs = args.sim2_min_docs if args.sim2_min_docs is not None else args.min_docs
    madoc_min_docs = args.madoc_min_docs if args.madoc_min_docs is not None else args.min_docs
    ...
    if len(sim2_texts) < sim2_min_docs:
        print(f"Simulation corpus too small: {len(sim2_texts)} < min_docs={sim2_min_docs}", file=sys.stderr)
    if args.madoc_merge_samples:
        madoc_texts, madoc_df, madoc_used_cols = load_madoc_samples(args.madoc_root, max_docs=madoc_max_docs, seed=args.seed, text_cols=madoc_text_cols)
    else:
        madoc_texts, madoc_df, madoc_used_cols = load_corpus(args.madoc_input, text_cols=madoc_text_cols, max_docs=madoc_max_docs, seed=args.seed)
    ...
    if len(madoc_texts) < madoc_min_docs:
        print(f"MADOC corpus too small: {len(madoc_texts)} < min_docs={madoc_min_docs}", file=sys.stderr)
    ...
    # Train BERTopic models with per-corpus configurations
    sim2_min_topic_size = args.sim2_min_topic_size if args.sim2_min_topic_size is not None else adaptive_min_topic_size(len(sim2_texts), args.min_topic_size)
    madoc_min_topic_size = args.madoc_min_topic_size if args.madoc_min_topic_size is not None else max(2, args.min_topic_size)
    sim2_vectorizer_min_df = args.sim2_vectorizer_min_df if args.sim2_vectorizer_min_df is not None else adaptive_min_df(len(sim2_texts), args.vectorizer_min_df)
    madoc_vectorizer_min_df = args.madoc_vectorizer_min_df if args.madoc_vectorizer_min_df is not None else max(1, args.vectorizer_min_df)
    sim2_min_token_freq = args.sim2_min_token_freq if args.sim2_min_token_freq is not None else args.min_token_freq
    madoc_min_token_freq = args.madoc_min_token_freq if args.madoc_min_token_freq is not None else args.min_token_freq
    sim2_df_threshold = args.sim2_df_threshold if args.sim2_df_threshold is not None else args.df_threshold
    madoc_df_threshold = args.madoc_df_threshold if args.madoc_df_threshold is not None else args.df_threshold
    ...
    if args.remove_contraction_fragments:
        common_extra_stop.update({"don", "ve", "ll", "re", "isn", "ain", "cant", "won", "shouldn", "wouldn", "couldn"})

    sim2_stopwords = build_stopword_set(
        sim2_texts,
        sim2_used_cols,
        common_extra_stop,
        min_token_freq=sim2_min_token_freq or 0.0,
        df_threshold=sim2_df_threshold or 0.0,
        min_docs_required=50,
    )
    madoc_stopwords = build_stopword_set(
        madoc_texts,
        madoc_used_cols,
        common_extra_stop,
        min_token_freq=madoc_min_token_freq or 0.0,
        df_threshold=madoc_df_threshold or 0.0,
        min_docs_required=200,
    )
    ...
    topic_model_sim2 = build_bertopic(
        embedding_model,
        min_topic_size=sim2_min_topic_size,
        seed=args.seed,
        vectorizer_min_df=sim2_vectorizer_min_df,
        hdbscan_kwargs=sim2_hdbscan_kwargs,
    )
    apply_stopwords(topic_model_sim2, sim2_stopwords)
    # Fit with fallback if vocabulary collapses
    try:
        _topics_sim2, _ = topic_model_sim2.fit_transform(sim2_texts)
    except ValueError as e:
        if "empty vocabulary" in str(e):
            # Create fresh model with extremely relaxed vectorizer and retry once
            print("Warning: empty vocabulary for sim2 topics; creating fresh model with extremely relaxed vectorizer (no stopwords, permissive pattern)", file=sys.stderr)
            topic_model_sim2 = build_bertopic(
                embedding_model,
                min_topic_size=sim2_min_topic_size,
                seed=args.seed,
            ...
            topic_model_sim2.vectorizer_model = CountVectorizer(
                ...
                min_df=1,
                token_pattern=r"(?u)\b\w+\b",  # Very permissive: any word
            )
            apply_stopwords(topic_model_sim2, sim2_stopwords)
            try:
                _topics_sim2, _ = topic_model_sim2.fit_transform(sim2_texts)
            except ValueError as e2:
                if "empty vocabulary" in str(e2):
                    print("Warning: Even extremely relaxed vectorizer failed; using fallback with minimal requirements", file=sys.stderr)
                    # Ultimate fallback: use the most basic vectorizer possible
                    topic_model_sim2.vectorizer_model = CountVectorizer(
                        stop_words="english",
                        ngram_range=(1, 1),
                        min_df=1,
                        token_pattern=r"(?u)\b[a-zA-Z]+\b",  # Only letters, no numbers
                    )
                    apply_stopwords(topic_model_sim2, sim2_stopwords)
                    try:
                        _topics_sim2, _ = topic_model_sim2.fit_transform(sim2_texts)
                    except ValueError as e3:
                        if "empty vocabulary" in str(e3):
                            print("Warning: Switching to SimpleTopicModel fallback for simulation corpus", file=sys.stderr)
                            topic_model_sim2 = SimpleTopicModel(seed=args.seed, min_topic_size=sim2_min_topic_size, top_n_words=args.top_n_words)
                            _topics_sim2, _ = topic_model_sim2.fit_transform(sim2_texts)
                        else:
                            raise
                else:
            ...
            raise
    topic_model_madoc = build_bertopic(
        embedding_model,
        min_topic_size=madoc_min_topic_size,
        seed=args.seed,
        vectorizer_min_df=madoc_vectorizer_min_df,
    )
    apply_stopwords(topic_model_madoc, madoc_stopwords)
    try:
        _topics_madoc, _ = topic_model_madoc.fit_transform(madoc_texts)
    except ValueError as e:
        if "empty vocabulary" in str(e):
            print("Warning: empty vocabulary for MADOC topics; creating fresh model with extremely relaxed vectorizer (no stopwords, permissive pattern)", file=sys.stderr)
            topic_model_madoc = build_bertopic(
                embedding_model,
                min_topic_size=madoc_min_topic_size,
                seed=args.seed,
            ...
            try:
                ...
                if "empty vocabulary" in str(e2):
                    print("Warning: Even extremely relaxed vectorizer failed for MADOC; using fallback with minimal requirements", file=sys.stderr)
                    # Ultimate fallback: use the most basic vectorizer possible
                    topic_model_madoc.vectorizer_model = CountVectorizer(
                        stop_words="english",
                        ngram_range=(1, 1),
                    ...
                    try:
                        ...
                        if "empty vocabulary" in str(e3):
                            print("Warning: Switching to SimpleTopicModel fallback for MADOC corpus", file=sys.stderr)
                            topic_model_madoc = SimpleTopicModel(seed=args.seed, min_topic_size=madoc_min_topic_size, top_n_words=args.top_n_words)
                            _topics_madoc, _ = topic_model_madoc.fit_transform(madoc_texts)
                        else:
                            raise
                else:
