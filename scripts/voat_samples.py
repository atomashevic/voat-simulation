...
NUMBER_OF_SAMPLES = 30  # Increased from 10 to 30
SECONDS_PER_DAY = 86400  # 24 * 60 * 60
OUTPUT_DIR = "MADOC/voat-technology"

# Time range constraints for sampling (2016-2020)
SAMPLE_START_YEAR = 2016
...
def create_time_samples(df):
    """
    Create NUMBER_OF_SAMPLES random 30-day samples from the specified time range (2016-2020).
    Uses random sampling with minimum gap between samples to ensure diversity.
    ...
    try:
        ...
        data_min_time = df['publish_date'].min()
        data_max_time = df['publish_date'].max()

        # Convert year constraints to timestamps
        from datetime import datetime
        constrained_start = datetime(SAMPLE_START_YEAR, 1, 1).timestamp()
        constrained_end = datetime(SAMPLE_END_YEAR, 12, 31, 23, 59, 59).timestamp()

        # Use the intersection of data range and constrained range
        min_time = max(data_min_time, constrained_start)
        max_time = min(data_max_time, constrained_end)

        # The latest possible start time for a sample (to ensure full 30 days)
        sample_window_seconds = SAMPLE_LENGTH_DAYS * SECONDS_PER_DAY
        latest_start = max_time - sample_window_seconds

        total_available_days = (latest_start - min_time) / SECONDS_PER_DAY

        logger.info(f"Data time span: {unix_to_date(data_min_time)} to {unix_to_date(data_max_time)}")
        logger.info(f"Constrained sampling range: {unix_to_date(min_time)} to {unix_to_date(max_time)}")
        logger.info(f"Available days for sampling start points: {total_available_days:.1f}")

        if latest_start <= min_time:
            raise ValueError(f"Not enough data in the constrained time range for {SAMPLE_LENGTH_DAYS}-day samples")

        # Generate random start times with minimum gap to reduce overlap
        # Minimum gap of 7 days between sample starts to ensure diversity
        MIN_GAP_DAYS = 7
        min_gap_seconds = MIN_GAP_DAYS * SECONDS_PER_DAY

        # Generate candidate start times
        sample_starts = []
        max_attempts = NUMBER_OF_SAMPLES * 100  # Prevent infinite loop
        attempts = 0

        while len(sample_starts) < NUMBER_OF_SAMPLES and attempts < max_attempts:
            attempts += 1
            # Generate a random start time
            candidate = np.random.uniform(min_time, latest_start)

            # Check if it's far enough from existing samples
            is_valid = True
            for existing_start in sample_starts:
                if abs(candidate - existing_start) < min_gap_seconds:
                    is_valid = False
                    break

            if is_valid:
                sample_starts.append(candidate)

        if len(sample_starts) < NUMBER_OF_SAMPLES:
            logger.warning(f"Could only generate {len(sample_starts)} samples with {MIN_GAP_DAYS}-day minimum gap")

        # Sort by time for easier interpretation
        sample_starts.sort()

        logger.info(f"Generated {len(sample_starts)} random sample start points")
        ...
        for i, sample_start in enumerate(sample_starts):
            ...
            if os.path.exists(sample_file_path):
                ...
                try:
                    ...
                    loaded_start = sample_df['publish_date'].min()
                    loaded_end = sample_df['publish_date'].max()

                    # Create sample info
                    sample_info = {
                        'sample_id': sample_id,
                        'start_date': unix_to_date(loaded_start),
                        'end_date': unix_to_date(loaded_end),
                        'start_timestamp': loaded_start,
                        'end_timestamp': loaded_end,
            ...
            sample_end = sample_start + sample_window_seconds
