...
def main(csv_path, use_lcc=False):
    ...
    try:
        ...
        with open(network_stats_path, 'w') as f:
            ...
            # Optionally filter to largest connected component for CP detection only
            if use_lcc:
                print("  - Using largest connected component for core-periphery detection...")
                largest_cc_for_cp = max(nx.connected_components(G), key=len)
                G_for_cp = G.subgraph(largest_cc_for_cp).copy()
                print(f"    * CP graph: {G_for_cp.number_of_nodes()} nodes, {G_for_cp.number_of_edges()} edges")
            else:
                G_for_cp = G

            # Run core-periphery detection
            cp_labels, mdl_value = run_sbm(G_for_cp)
            ...
            denom_nodes = G_for_cp.number_of_nodes() if use_lcc else G.number_of_nodes()
            print(f"- Core Size: {core_count} nodes ({(core_count/denom_nodes)*100:.2f}% of CP graph)")
            print(f"- Periphery Size: {periphery_count} nodes ({(periphery_count/denom_nodes)*100:.2f}% of CP graph)")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--lc', '--largest-component', dest='use_lcc', action='store_true',
                        help='Run core-periphery detection on the largest connected component only')
    args = parser.parse_args()

    main(args.csv_path, use_lcc=args.use_lcc) 
